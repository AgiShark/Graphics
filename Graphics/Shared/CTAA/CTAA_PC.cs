...
namespace Graphics.CTAA
{
    ...
    public class CTAA_PC : MonoBehaviour
    {
        ...
        void Awake()
        {
            ...
            if (ctaaMat == null) ctaaMat = CreateMaterial("assets/shaders/ctaa_pc.shader");
            if (mat_enhance == null) mat_enhance = CreateMaterial("assets/shaders/ctaa_enhance_pc.shader");
        ...
        private void Start()
        {
            ...
            if (ExtendedFeatures)
            {
                ...
                print("CTAA Extended Mode Enabled");
                //--------------------------------------------------------------------------------------------------
                ...
                if (((upScaleRT == null) || (upScaleRT.width != sizex)) || (upScaleRT.height != sizey))
                {
                    ...
                    print("SuperResolution Changed And Created" + upScaleRT.width + "," + upScaleRT.height);
                ...
                if (!m_LayerRenderCam)
                {
                    ...
                    if (!shaders.TryGetValue("assets/shaders/ctaa_enhance_pc.shader", out Shader maskShader))
                    {
                        maskShader = assetBundle.LoadAsset<Shader>("assets/shaders/ctaa_enhance_pc.shader");
                        shaders.Add("assets/shaders/ctaa_enhance_pc.shader", maskShader);
                    }

                    rpCTAA.maskRenderShader = maskShader;

                    if (!shaders.TryGetValue("assets/shaders/ctaa_enhance_pc.shader", out Shader enhanceShader))
                    {
                        enhanceShader = assetBundle.LoadAsset<Shader>("assets/shaders/ctaa_enhance_pc.shader");
                        shaders.Add("assets/shaders/ctaa_enhance_pc.shader", enhanceShader);
                    }

                    rpCTAA.layerPostMat = new Material(enhanceShader);
            ...
            if (startResX != Screen.width || startResY != Screen.height)
            {
                ...
                if (ExtendedFeatures)
                {
                    ...
                    if (((upScaleRT == null) || (upScaleRT.width != sizex)) || (upScaleRT.height != sizey))
                    {
                        ...
                        upScaleRT.Create();
                        GetComponent<Camera>().targetTexture = upScaleRT;
                        print("CTAA SuperResolution Updated");
                        print("CTAA SuperResolution Updated" + upScaleRT.width + "," + upScaleRT.height);
                    }
            ...
            if (CTAA_Enabled)
            {
                ...
                if (PreEnhanceEnabled)
                {
                    ...
                    if (swap)
                    {
                        ...
                        UnityEngine.Graphics.Blit(afterPreEnhace, rtAccum1, ctaaMat);
                        UnityEngine.Graphics.Blit(rtAccum1, destination);
                    }
                    else
                    {
                        ctaaMat.SetTexture("_Accum", rtAccum1);
                        UnityEngine.Graphics.Blit(afterPreEnhace, rtAccum0, ctaaMat);
                ...
                else
                {
                    ...
                    {
                        ctaaMat.SetTexture("_Accum", rtAccum0);
                        UnityEngine.Graphics.Blit(source, rtAccum1, ctaaMat);
                        UnityEngine.Graphics.Blit(rtAccum1, destination);
                    }
                    else
                    {
                        ctaaMat.SetTexture("_Accum", rtAccum1);
                        UnityEngine.Graphics.Blit(source, rtAccum0, ctaaMat);
                        UnityEngine.Graphics.Blit(rtAccum0, destination);
                    }
